...
class TTFLiteHead(TTFHead):
    ...
    def __init__(self,
        ...
        super(TTFLiteHead, self).__init__(
            ...
            wh_head_conv_num=wh_head_conv_num,
    ...
    def upsample(self, x, out_c, name=None):
        deconv_up = self._deconv_upsample(x, out_c // 2, name=name + '.dilation_up')
        interp_up = self._interp_upsample(x, out_c // 2, name=name + '.interp_up')
        # interp_up = self._interp_upsample(x, out_c, name=name + '.interp_up')
        # return deconv_up + interp_up
        # return interp_up
        return fluid.layers.concat([deconv_up, interp_up], axis=1)
